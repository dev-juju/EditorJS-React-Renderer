...
const HeaderOutput = ({ data, style, classNames, config }: ErrRendererProps) => {
  ...
  if (typeof data === 'object' && data.level && typeof data.level === 'number') {
    switch (data.level) {
      case 1: return <h1 style={ headerStyle } className={ classNames }>{ parse(content) }</h1>;
      case 2: return <h2 style={ headerStyle } className={ classNames }>{ parse(content) }</h2>;
      case 3: return <h3 style={ headerStyle } className={ classNames }>{ parse(content) }</h3>;
      case 4: return <h4 style={ headerStyle } className={ classNames }>{ parse(content) }</h4>;
      case 5: return <h5 style={ headerStyle } className={ classNames }>{ parse(content) }</h5>;
      case 6: return <h6 style={ headerStyle } className={ classNames }>{ parse(content) }</h6>;
      default: return <h4 style={ headerStyle } className={ classNames }>{ parse(content) }</h4>;
    }
  }

  return <h4 style={ headerStyle } className={ classNames }>{ parse(content) }</h4>;
};
