...
const TableOutput = ({
  ...
  const tableStyle = config.disableDefaultStyle
    ? style.table
    : { ...tableOutputStyle.table, ...style.table };
  const trStyle = config.disableDefaultStyle
    ? style.tr
    : { ...tableOutputStyle.tr, ...style.tr };
  const thStyle = config.disableDefaultStyle
    ? style.th
    : { ...tableOutputStyle.th, ...style.th };
  const tdStyle = config.disableDefaultStyle
    ? style.td
    : { ...tableOutputStyle.td, ...style.td };
      ...
      <thead>
        <tr style={trStyle} className={classNames.tr}>
          {columnNames.map((columnName, index) => (
            <th key={index} style={thStyle} className={classNames.th}>
              {parse(columnName)}
            </th>
          ))}
        </tr>
          ...
          >
            {Array.isArray(row) &&
              ...
              row.map((columnValue, i) => (
                <td key={i} style={tdStyle} className={classNames.td}>
                  {parse(columnValue)}
                </td>
              ))}
          </tr>
